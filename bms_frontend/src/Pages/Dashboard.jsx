...
export default function Dashboard(props) {
  ...
  useEffect(() => {
    axios
      .get(`${baseUrl}books/top-n-books/${n}`)
      .then((response) => {
        console.log(
          `Top 5 books data:${JSON.stringify(response.data.booksList)}`
        );
        setTopBooksList(response.data.booksList);
      })
      .catch((err) => {
  ...
  const createAddToCartReq = async (params) => {
    ...
    const response = await axios.post(
      `${baseUrl}cart/add-to-cart-books`,
      {
        add_to_cart_data: arr,
      },
